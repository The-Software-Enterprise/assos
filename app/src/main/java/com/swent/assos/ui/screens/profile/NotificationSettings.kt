...
fun NotificationSettings(navigationActions: NavigationActions) {
  Scaffold(
      modifier =
          Modifier.semantics { testTagsAsResourceId = true }.testTag("NotificationSettingsScreen"),
    ...
    Column(modifier = Modifier.padding(paddingValues).fillMaxWidth()) {
      ...
      BasicButtonWithSwitch(
          "Alert to staff or join associations", "Be notified on new open positions")
    }
  }
}
